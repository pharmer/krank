...
import (
	...
	"github.com/appscode/krank/templates"
...
func (ks *KubeScript) Perform() error {
	...
	if ks.Req.Role == api.RoleKubernetesMaster {
	...
	if ks.Req.Role == api.RoleKubernetesPool || ks.Req.RegisterMasterKubelet {
...
func (ks *KubeScript) AssignHostname() error {
	...
	if firebaseUid != "" {
		...
		path := fmt.Sprintf("/kubernetes/v1beta1/clusters/%v/instance-by-ip/%v/json", "" /*ks.Req.UID*/, externalIP)
		_, err = httpclient.New(nil, nil, nil).
			WithBaseURL("" /*api.PublicAPIHttpEndpoint()*/).
	...
	var resp proto.ClusterInstanceByIPResponse
...
func (ks *KubeScript) WriteEnv() error {
	...
	ks.WriteBytes("/var/cache/kubernetes-install/kube_api.yaml", data)
	return nil
}
...
func (ks *KubeScript) DownloadApps() {
	//ks.Req.Apps[api.AppKubeServer].Download(ks.AppDir())
	//ks.Req.Apps[api.AppKubeSaltbase].Download(ks.AppDir())
	//ks.Req.Apps[api.AppHostfacts].Download(ks.AppDir())
}
...
func (ks *KubeScript) ConfigureSalt() {
	...
	} else if ks.Req.Role == api.RoleKubernetesPool {
...
func (ks *KubeScript) InstallSaltBase() {
	...
	//saltbasePath := ks.AppDir() + "/" + ks.Req.Apps[api.AppKubeSaltbase].Name
	//ks.Shell.Command("tar", "-xz", "-C", "/srv", "-f", saltbasePath, "--strip", "2").Run()
	//ks.CheckPathExists(saltbasePath)
	//ks.Shell.Command("/srv/install.sh", ks.AppDir()+"/"+ks.Req.Apps[api.AppKubeServer].Name).Run()
...
func (ks *KubeScript) InstallHostfacts() {
	//o := ks.AppDir() + "/" + ks.Req.Apps[api.AppHostfacts].Name
	//n := "/usr/bin/" + ks.Req.Apps[api.AppHostfacts].Name
