...
var KubeStarters = &kubeStarterExt{
	newExtensionPoint(new(KubeStarter)),
}

type kubeStarterExt struct {
	*extensionPoint
}

func (ep *kubeStarterExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *kubeStarterExt) Register(extension KubeStarter, name string) bool {
	return ep.register(extension, name)
}

func (ep *kubeStarterExt) Lookup(name string) KubeStarter {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(KubeStarter)
}

func (ep *kubeStarterExt) Select(names []string) []KubeStarter {
	var selected []KubeStarter
...
func (ep *kubeStarterExt) All() map[string]KubeStarter {
	all := make(map[string]KubeStarter)
...
func (ep *kubeStarterExt) Names() []string {
	var names []string
	for k := range ep.all() {
