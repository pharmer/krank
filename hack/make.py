...
libbuild.REPO_ROOT = expandvars('$GOPATH') + '/src/github.com/appscode/krank'
...
def fmt():
    libbuild.ungroup_go_imports('*.go', 'cloud', 'cmds', 'templates')
    die(call('goimports -w *.go cloud cmds templates'))
    call('gofmt -s -w *.go cloud cmds templates')
...
def gen_assets():
    die(call('go-bindata -ignore=\\.go -ignore=\\.DS_Store -mode=0644 -modtime=1453795200 -o bindata.go -pkg templates .', cwd=libbuild.REPO_ROOT + '/templates'))
